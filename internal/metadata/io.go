...
func WriteYAML(data interface{}, filename string) error {
	...
	file, err := os.OpenFile(filename, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, filePerm) //nolint:gosec // filename is derived from internal config paths
	if err != nil {
		return err
	}
	defer func() {
...
func LoadMetadata(filename string) (*Metadata, error) {
	data, err := os.ReadFile(filename) //nolint:gosec // filename is a user-provided metadata path
	if err != nil {
		return nil, err
	}
...
func LoadYAML(filename string, data interface{}) error {
	fileData, err := os.ReadFile(filename) //nolint:gosec // filename is a user-provided YAML path
	if err != nil {
		return err
	}
