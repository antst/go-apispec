...
// resolveIdentType must prefer the fully-qualified TypeRef over the unqualified
// pooled Type (FIX 6), while still honouring a bare type-parameter fallback and
// the pooled-Type / variable-lookup paths when no usable TypeRef is present.
func TestResolveIdentType_PrefersTypeRef(t *testing.T) {
	meta := newTestMeta()
	r := NewTypeResolver(meta, nil)

	// TypeRef qualified, pooled Type bare -> qualified wins.
	arg := metadata.NewCallArgument(meta)
	arg.SetKind(metadata.KindIdent)
	arg.SetName("u")
	arg.SetType("User") // unqualified pooled type
	arg.TypeRef = &metadata.TypeRef{Kind: metadata.RefNamed, Pkg: "github.com/x/models", Name: "User"}
	assert.Equal(t, "github.com/x/models.User", r.resolveIdentType(*arg))

	// A bare type-parameter ref (RefParam) carries no concrete type, so the pooled
	// Type is the better answer and must be used instead.
	argParam := metadata.NewCallArgument(meta)
	argParam.SetKind(metadata.KindIdent)
	argParam.SetName("v")
	argParam.SetType("Concrete")
	argParam.TypeRef = &metadata.TypeRef{Kind: metadata.RefParam, Name: "T"}
	assert.Equal(t, "Concrete", r.resolveIdentType(*argParam))

	// No TypeRef, pooled Type present -> pooled Type (unchanged legacy behaviour).
	argNoRef := metadata.NewCallArgument(meta)
	argNoRef.SetKind(metadata.KindIdent)
	argNoRef.SetName("w")
	argNoRef.SetType("Plain")
	assert.Equal(t, "Plain", r.resolveIdentType(*argNoRef))

	// A func-typed ref stringifies to the bare "func" (the signature is lost), but
	// the pooled Type still carries the full "func(...)..." that resolveCallType
	// needs to extract a callee's return type — so the pooled Type must win.
	argFunc := metadata.NewCallArgument(meta)
	argFunc.SetKind(metadata.KindIdent)
	argFunc.SetName("fn")
	argFunc.SetType("func(int) error")
	argFunc.TypeRef = &metadata.TypeRef{Kind: metadata.RefFunc}
	assert.Equal(t, "func(int) error", r.resolveIdentType(*argFunc),
		"func ref String() is lossy; pooled signature must win")

	// Likewise a chan-typed ref stringifies to a bare "chan"; prefer the pooled type.
	argChan := metadata.NewCallArgument(meta)
	argChan.SetKind(metadata.KindIdent)
	argChan.SetName("ch")
	argChan.SetType("chan int")
	argChan.TypeRef = &metadata.TypeRef{Kind: metadata.RefChan}
	assert.Equal(t, "chan int", r.resolveIdentType(*argChan),
		"chan ref String() is lossy; pooled type must win")
}

// typeRefStringIsLossy gates the FIX 6 TypeRef preference: only the kinds whose
// String() drops information (param/func/chan) fall back to the pooled type.
func TestTypeRefStringIsLossy(t *testing.T) {
	for _, k := range []metadata.RefKind{metadata.RefParam, metadata.RefFunc, metadata.RefChan} {
		assert.True(t, typeRefStringIsLossy(k), "kind %v stringifies to a bare keyword", k)
	}
	for _, k := range []metadata.RefKind{
		metadata.RefBasic, metadata.RefNamed, metadata.RefSlice, metadata.RefArray,
		metadata.RefMap, metadata.RefPointer, metadata.RefInterface, metadata.RefStruct,
	} {
		assert.False(t, typeRefStringIsLossy(k), "kind %v stringifies to a usable form", k)
	}
}
